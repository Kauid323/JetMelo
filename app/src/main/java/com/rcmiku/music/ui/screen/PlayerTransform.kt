...
fun PlayerTransform(
    ...
    var isAnimatingCommit by remember { mutableStateOf(false) }
            ...
            .pointerInput(mediaMetadata) {
                detectHorizontalDragGestures(
                    onHorizontalDrag = { _, dragAmount ->
                        if (!isAnimatingCommit) dragSumX += dragAmount
                    },
                    onDragEnd = {
                        val shouldNext = dragSumX <= -swipeThresholdPx
                        val shouldPrev = dragSumX >= swipeThresholdPx

                        if (shouldNext || shouldPrev) {
                            isAnimatingCommit = true
                            val target = if (shouldNext) -1000f else 1000f
                            dragSumX = target
                            if (shouldNext) mediaController?.seekToNext() else mediaController?.seekToPrevious()
                            dragSumX = 0f
                            isAnimatingCommit = false
                        } else {
                            dragSumX = 0f
                        }
                    },
            ...
            when (it) {
                FULL_PLAYER -> {
                    Player(
                        ...
                        modifier = contentModifier.sharedBounds(
