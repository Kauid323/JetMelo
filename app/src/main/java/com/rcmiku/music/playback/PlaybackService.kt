                ...
                val dataSourceFactory: ResolvingDataSource.Factory = ResolvingDataSource.Factory(
                    ...
                    runBlocking {
                        val resolved = runCatching {
                            ...
                            Log.e("PlaybackService", "updateMediaItemUri failed songId=${dataSpec.uri.path}", e)
                            throw PlaybackException(
                                "updateMediaItemUri failed",
                                e,
                                PlaybackException.ERROR_CODE_REMOTE_ERROR
                            )
                        }
                        if (resolved == null) {
                            Log.e("PlaybackService", "updateMediaItemUri returned null songId=${dataSpec.uri.path}")
                            throw PlaybackException(null, null, PlaybackException.ERROR_CODE_REMOTE_ERROR)
                        }
                        Log.w("PlaybackService", "resolved play url=$resolved")
                        dataSpec.withUri(resolved)
                    }
                }
