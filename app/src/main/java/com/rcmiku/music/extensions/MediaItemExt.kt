...
fun List<CloudSong>.toCloudSongMediaItemList(uid: Long) =
    this.map { cloudSong ->
        MediaItem.Builder()
            ...
            .setMediaMetadata(
                MediaMetadata.Builder()
                    .setArtist(cloudSong.simpleSong.ar.joinToString("/") { it.name ?: "" })
...
suspend fun updateMediaItemUri(songId: String, songLevel: SongLevel): Uri? {
    val result = PlayerApi.songPlayUrlV1(songId, songLevel = songLevel)
    val resp = result.getOrNull()
    if (resp == null) {
        Log.e(
            "MediaItemExt",
            "updateMediaItemUri api failed songId=$songId level=${songLevel.level}",
            result.exceptionOrNull()
        )
    }
    val url = resp?.data?.firstOrNull { !it.url.isNullOrBlank() }?.url
    Log.w(
        "MediaItemExt",
        "updateMediaItemUri songId=$songId level=${songLevel.level} code=${resp?.code} urlPresent=${!url.isNullOrBlank()}"
    )
    return url?.toUri()
}